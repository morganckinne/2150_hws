...
; Parameter 3 is the right index in the array (rdx)  rbp is extra register
; Return type is void
mergeSort:
	; Implement mergeSort here
    cmp     rsi, rdx        ; if(start == end)
    jge     return          ; jump to end of loop1, where it returns the separated value happens
    mov     r10, rsi        ; r10 = rsi
    add     r10, rdx        ; r10 += rdx
    shr     r10,1           ; r10/=2
    push    r10
    push    rdx             ; rdx stored on the stack mergesort(rsi, rdx)
    mov     rdx, r10        ; rdx = r10
    call    mergeSort       ; mergesort(rsi, r10)
    pop     rdx             ;
    pop     r10             ;
    push    rsi             ; rsi stored on the stack
    mov     rsi, r10        ; rsi = r10
    push    r10             ;
    inc     rsi             ;
    call    mergeSort       ; mergesort(r10++, rdx)
    pop     r10             ;
    pop     rsi             ;
    push    rdx             ;
    push    r10             ;
    mov     rcx, rdx        ; end = end
    mov     rdx, r10        ; middle var = mid
    call    merge           ; merge
    pop     r10             ;
    pop     rdx             ;
return:
